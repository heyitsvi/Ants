...
class Insect:
    ...
    def reduce_health(self, amount):
        ...
        3
        """
        # print("DEBUG: self :", self.place)
        self.health -= amount
        if self.health <= 0 and self.place != None:
            self.death_callback()
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while current_place.is_hive == False:
            if current_place.bees != [] and ((self.min_range <= count <= self.max_range) == True):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        #         damage_bees(self.place.bees, amount)
        #     else:
        #         cls.reduce_health(amount)
        # helper_func()

        list_copy = list(self.place.bees)
        # print("DEBUG: damage amount", self.damage)
        # print("DEBUG: health", self.health)
        # print("DEBUG: self:", self)
        # print("DEBUG: amount:", amount)
        if amount >= self.health:
            # print("DEBUG: Original Bee List", len(list_copy))
            # for bee in list_copy:
            #     print("DEBUG: Bee health before", bee.health)
            for bee in list_copy:
                Ant.reduce_health(bee, amount)
            # print("DEBUG: Mutated Bee List", len(list_copy))

            # if None not in list_copy:             
            for bee in list_copy:
                # print("DEBUG: Bee", bee)
    ...
    def action(self, gamestate):
        ...
        if self.chew_countdown != 0:
            ...
            if self.place.bees != []:
                ...
                print("DEBUG: Place", self.place.bees)

                while bee.health > 0:
                    Ant.reduce_health(bee, 1)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        # print("DEBUG:", list_copy)
        for bee in list_copy:
            Ant.reduce_health(bee, self.damage)
        # print("DEBUG: 2", list_copy)
        if self.ant_contained != None:
            contained_ant = self.ant_contained
            contained_ant.action(gamestate)
# END Problem 9
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        its health to 0."""
        # BEGIN Problem 10
        print("DEBUG:", insect.place)
        Place.add_insect(self, insect)
        print("DEBUG:", insect.place)
        print("DEBUG:", insect.is_waterproof)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # print("DEBUG: cls", gamestate)
        # print("DEBUG:",gamestate.queen_bee_instance)
        if gamestate.queen_bee_instance == False:
            gamestate.queen_bee_instance = True
            return super().construct(gamestate)    
        else:
            return None
        # print("DEBUG:", cls.queen_bee_instance)
    ...
    def action(self, gamestate):
        ...
        # print("DEBUG:", previous_place)

        while previous_place != None:
            # print("DEBUG: ant",previous_place.ant)
            if (previous_place.ant):
                if not previous_place.ant.damage_doubled:
                    previous_place.ant.damage *= 2
                    previous_place.ant.damage_doubled = True
                # print("DEBUG: damage_double", previous_place.ant.damage_doubled)
                # print("DEBUG: ant isinstance",previous_place.ant, isinstance(previous_place.ant, ContainerAnt))
                if isinstance(previous_place.ant, ContainerAnt):
                    # print("DEBUG:", "I am the container ant")
                    guard_ant = previous_place.ant
                    guarded_ant = guard_ant.ant_contained
                    # guard_ant.damage *= 2
                    # guard_ant.damage_doubled = True
                    if guarded_ant and guarded_ant.damage_doubled == False:
                        # print("DEBUG: previous damage",guarded_ant.damage)
                        if isinstance(guarded_ant,FireAnt):
                            return

                        guarded_ant.damage *= 2
                        guarded_ant.damage_doubled = True
                        # print("DEBUG: present damage",guarded_ant.damage)
                    # print("DEBUG: returned ant",guarded_ant)
